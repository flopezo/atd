---
title: "Introduction to ggplot and rOpenSci"
...
Basic usage: `qplot(x, y, data...)`, where data is the data frame to use `...` means other aesthetics passed for each layer.
...
polistesdat.url <- "https://raw.githubusercontent.com/flopezo/atd/master/Tibbets_et_al_2015_data.csv"
...
qplot(logface, nest.size, data=polistes.data) + geom_point(size=4)
qplot(logface, nest.size, data=polistes.data, shape=sfmf) + geom_point(size=4)

# Add colors based on a categorical variable
qplot(logface, nest.size, data=polistes.data, colour=sfmf) + geom_point(size=4)
```
...
qplot(logface, nest.size, data=polistes.data, geom=c("point", "smooth"), 
  ...
  ylab="NUmber of Nest Cells") + geom_point(size=4)
```
...
# `ggplot`
`qplot` does not show the power of `ggplot`. `ggplot2` functions can be chained with "+" signs. All the options that can be chained are available at  [docs.ggplot2.org](http://docs.ggplot2.org/current/). Let us remake the previous graphs using a few of the wide variety of options available in `ggplot`.
## Scatterplots
```{r}
ggplot(polistes.data, aes(x=weight, y=nest.size, colour=nest.size)) + 
  ...
  labs(title="Relationship between weight and nest size", 
    x="Weight", y="Number of Cells") +
  # Add text to graph
  annotate("text", x=0.130, y=75, label="Group 1", size=10) +
  annotate("text", x=0.075, y=180, label="Group 2", size=10) + 
...
ggplot(cw, aes(x=Diet, y=weight, colour=Diet)) + geom_boxplot() + 
  ...
  labs(title="Relationship between diet and weight", 
    x="Diet", y="Weight") +
  theme(plot.title=element_text(size=rel(2))) + # Title twice the base font size
...
ggplot(cwse, aes(x=Diet, y=weightm, fill=Diet)) + 
  ...
  geom_errorbar(aes(ymin=weightm-se, ymax=weightm+se), width=.2) + 
...
## Histograms
```{r}
ggplot(polistes.data, aes(x=weight, fill=sfmf)) + geom_histogram(alpha=.8)
ggplot(cw, aes(x=weight, fill=Diet)) + geom_histogram(alpha=.6)
...
ggplot(cw, aes(x=weight, fill=Diet)) + geom_histogram(alpha=.5) + 
...
geom_point(aes(x=decimalLongitude, y=decimalLatitude, group=name), 
  colour="orange", size=4, data=apicea.lat.lon)
...
bombus.map.occ <- geom_point(aes(x=decimalLongitude, y=decimalLatitude, 
  group=name, colour=name, shape=name), size=4, data=bombus.lat.lon)
...
bombus.maplegend <- theme(plot.title=element_text(face="italic", 
  ...
  theme(legend.text=element_text(face="italic", size=rel(1.2))) +
...
bombus.map.den <-  stat_density2d(aes(x=decimalLongitude, y=decimalLatitude, 
  group=name, colour=name), size=1, data=bombus.lat.lon, geom="density2d")
